def grammar_check(spelling_output):
    ...
    def preprocess_data(df, tokenizer, max_input_length=128, max_target_length=8):
        ...
        #adding task prompt prefix for clarifying of the model
        ...
        #tokenizing target sentences
        model_inputs["labels"] = labels["input_ids"]

        model_inputs["labels"][labels["attention_mask"] == 0] = -100 #replacing padding tokens with -100 so that the model ignores it while training

        return model_inputs

    tokenized_data = preprocess_data(df, tokenizer)
    #tokenized incorrect and correct sentences acc to above fxn
    #splitting and tokenized data into training and validation and randomizing them 
    ...
    #checking for gpu and will fall back on cpu if gpu is unavailable
    train_dataset = TensorDataset(train_input_ids, train_attention_mask, train_labels)
    val_dataset = TensorDataset(val_input_ids, val_attention_mask, val_labels)
    data_collator = DataCollatorForSeq2Seq(tokenizer=tokenizer, model=model) #for dynamic padding

    training_args = Seq2SeqTrainingArguments(
        output_dir="./results",
        num_train_epochs=3, #total numbner of training iterations
        per_device_train_batch_size=16,
        per_device_eval_batch_size=16,
        warmup_steps=500,
        weight_decay=0.01,
        logging_dir="./logs",
        logging_steps=10, #storing log metrics every ten steps
        predict_with_generate=True,
        report_to='none' #explicitly diables WANDB
    )

    class CustomT5Dataset(Dataset): #extra function for quick transformations
        ...
        def __getitem__(self, idx): #returns outputs compatible with HF trainer(used later in code by LLaMA
            return {
    ...
    metric = evaluate.load("accuracy") # unpack predictions passed by Trainer
    ...
    def compute_metrics(eval_preds):
        ...
        if isinstance(preds, tuple):
            preds = preds[0]
        decoded_preds = tokenizer.batch_decode(preds, skip_special_tokens=True)#decoding all the tokens into a suitable output
    ...
    if TRAIN: #to run training and evaluation parts
        trainer.train()
        model.save_pretrained("saved_grammar_model")
        tokenizer.save_pretrained("saved_grammar_model")
    else:
        model = T5ForConditionalGeneration.from_pretrained("saved_grammar_model") #or loads already trained model if present, reduces redundant training
